...
├── nginx.conf                        # Reverse-proxy routing (port 80 → services)
...
All services are defined in `docker-compose.yml` and share an internal Docker network. Postgres health checks gate all dependent services. Nginx acts as the single host-facing entry point on port 80, routing traffic to the correct upstream by URL prefix.
...
| `nginx` | `nginx:alpine` | **`80`** | Reverse proxy — unified entry point |

**Nginx routing (nginx.conf):**

| Path prefix | Upstream | Behaviour |
|-------------|----------|-----------|
| `/api/*` | `api-server:8000` | `/api/` prefix stripped before forwarding |
| `/airflow/*` | `airflow-webserver:8080` | Full prefix preserved (Airflow mounts routes under `/airflow`) |
| `/*` | `neo-app:3000` | Catch-all for the Next.js frontend |
...
- `./nginx.conf` → `/etc/nginx/nginx.conf` (read-only bind mount)
...
> When running via Docker Compose these values are set directly in `docker-compose.yml`. The `.env` file is only needed when running the Next.js app locally outside of Docker.
...
4. Start the Airflow webserver, scheduler, API server, Next.js app, and Nginx gateway
...
Navigate to [http://localhost/airflow](http://localhost/airflow) and log in with:
- **Username:** `airflow`
- **Password:** `airflow`
...
All user-facing services are reachable through the Nginx gateway on **port 80**:

| Service | URL (via Nginx) | Direct Port | Credentials |
|---------|----------------|-------------|-------------|
| **Web App** | [http://localhost](http://localhost) | `localhost:3000` | Register via `/signup` |
| **API Docs** | [http://localhost/api/docs](http://localhost/api/docs) | `localhost:8000/docs` | — |
| **Airflow UI** | [http://localhost/airflow](http://localhost/airflow) | `localhost:8080/airflow` | `airflow` / `airflow` |
...
### `POST /api/neo/predict-batch`

Run predictions on up to 100 asteroids in a single request.

```bash
curl -X POST http://localhost/api/neo/predict-batch \
  -H "Content-Type: application/json" \
  -d '[
...
```bash
curl http://localhost/api/neo/stats
```

### `GET /api/neo/search?name=<query>`

```bash
curl "http://localhost/api/neo/search?name=2000+SS&limit=10"
```

Full interactive documentation with try-it-out support is available at **[http://localhost/api/docs](http://localhost/api/docs)**.
...
| `NEXTAUTH_URL` | `http://localhost` | Canonical URL of the app (nginx entry point) |
| `NEXT_PUBLIC_API_URL` | `http://localhost/api` | API base URL baked into the client bundle at build time — must use the nginx-proxied path |
...
|------------|---------|------|
| scikit-learn | 1.3.2 | Model training, scalers, metrics |
...
| dill | 0.3.1.1 | Extended pickle support for joblib artifacts |
...
| Nginx | alpine | Reverse proxy — unified port-80 entry point |
